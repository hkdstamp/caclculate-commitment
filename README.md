...
このツールは、AWSのコストデータ（CSV形式）をアップロードすることで、Reserved InstanceとSavings Plansを適用した場合のコスト削減額、スマート予約利用料、最終支払額、実効割引率を自動計算します。
...
- ✅ **スマート予約利用料計算**: 30日コミットメント保証（50%）と1年コミットメント保証（30%）のスマート予約利用料を含めた計算
...
- **サマリー**: RI/SPそれぞれの総コスト、削減額、スマート予約利用料、最終支払額
...
**最終支払額への組み込み**:
```
最終支払額 = コミットメントコスト + 月額初期費用 + スマート予約利用料
```
...
- **月額初期費用**: $54,720 / 36 = **$1,520**
- **最終支払額**: $3,100.992 + $1,520 + スマート予約利用料
...
- `ri_insurance_30d`, `ri_insurance_1y`: スマート予約利用料
